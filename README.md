...
The traditional approach of representing entities within games is to use an inheritance hierarchy. An inheritance hierarchy is an approach where classes can completely inherit the member variables and methods of other classes. The Unreal Game Engine uses an inheritance hierarchy where "Character" derives from "Pawn" which derives from "Actor".  This allows for members that are common to all "Actors" to also be shared with all characters within the game. The problem with inheritance is that all members within a parent class must be inherited by the derived classes in it's entirety, it is not possible to "partially inherit" a class. This can lead to the diamond problem, where it can be difficult to create classes that need to be a combination of multiple inheritance hierarchies. This approach can also lead to bloat, since there may be members in the parent class that are unused in some of the derived classes. So, generally the advice used nowadays is to prefer "composition over inheritance" where objects are instead made of parts (or components) that can be reused in different places. This allows reusable code similar to an inheritance hierarchy but the reuse can now be partial - only the data and code that is needed can be added to the new object. 
